...
func TestReconcileVPCSubnets(t *testing.T) {
	...
	t.Run("When VPCSubnets are set in spec", func(t *testing.T) {
		...
		g.Expect(err).To(BeNil())
		g.Expect(clusterScope.IBMPowerVSCluster.Status.VPCSubnet["subnet1Name"].ID).To(Equal(subnet1Details.ID))
		g.Expect(clusterScope.IBMPowerVSCluster.Status.VPCSubnet["subnet2Name"].ID).To(Equal(subnet2Details.ID))
	})

	t.Run("When VPCSubnets are not set in spec and vpc doesnt exist already, create new", func(t *testing.T) {
		g := NewWithT(t)
		setup(t)
		t.Cleanup(teardown)
	...
	t.Run("When subnetID and subnetName is nil", func(t *testing.T) {
		...
		clusterScope := PowerVSClusterScope{
			...
			IBMPowerVSCluster: &infrav1beta2.IBMPowerVSCluster{
				...
				Status: infrav1beta2.IBMPowerVSClusterStatus{
					VPCSubnet: map[string]infrav1beta2.ResourceReference{
						"subnet1Name": infrav1beta2.ResourceReference{ID: ptr.To("subnet1ID")},
					}},
			},
		}
		subnet1Details := &vpcv1.Subnet{ID: ptr.To("subnet1ID"), Name: ptr.To("subnet1Name")}
		...
		},
		{
			...
			clusterScope: PowerVSClusterScope{
				IBMPowerVSCluster: &infrav1beta2.IBMPowerVSCluster{
					Status: infrav1beta2.IBMPowerVSClusterStatus{
						VPCSubnet: map[string]infrav1beta2.ResourceReference{
							"subnet1Name": infrav1beta2.ResourceReference{
...
func TestCheckVPCSubnet(t *testing.T) {
	...
	t.Run("When GetVPCSubnetByName returns empty as vpc subnet doesnt exist", func(t *testing.T) {
		g := NewWithT(t)
		setup(t)
		t.Cleanup(teardown)
